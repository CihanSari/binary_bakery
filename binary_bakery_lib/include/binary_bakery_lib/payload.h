...
namespace bb {
   ...
   struct payload {
      ...
      payload(std::vector<uint8_t>&& content, const content_meta& meta, const file_path& file)
   ...
   [[nodiscard]] auto get_payload(const file_path& path, const config& cfg) -> payload;
