...
namespace tests {
  ...
  const std::vector<uint8_t> expected =
      get_image_bytes(file_path{testRoot / "test_images/green.png"});
      const std::vector<uint8_t> bytes_from_payload = get_decode_to_vector_result<test_rgb>(get_payload("green.png"), zstd_decompression);
   ...
   TEST_CASE("decode_to_vector, generic binary")
   {
     const std::vector<uint8_t> expected = get_binary_file(
         file_path{testRoot / "test_images/binary0.bin"});
      const std::vector<uint8_t> bytes_from_payload = decode_to_vector<uint8_t>(get_payload("binary0.bin"), zstd_decompression);
      CHECK_EQ(expected, bytes_from_payload);
   }
   ...
   TEST_CASE("decode_into_pointer, image")
   {
     const std::vector<uint8_t> bytes_from_file =
         get_image_bytes(file_path{testRoot / "test_images/green.png"});
      const std::vector<uint8_t> bytes_from_payload = get_decode_into_pointer_result(get_payload("green.png"), zstd_decompression);
   ...
   TEST_CASE("decode_into_pointer, generic binary")
   {
     const std::vector<uint8_t> bytes_from_file = get_binary_file(
         file_path{testRoot / "test_images/binary0.bin"});
      const std::vector<uint8_t> bytes_from_payload = get_decode_into_pointer_result(get_payload("binary0.bin"), zstd_decompression);
      CHECK_EQ(bytes_from_file, bytes_from_payload);
   }
